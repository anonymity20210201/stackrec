...
def main():
    ...
    parser.add_argument('--save_dir', default='Models/ML20_baseline_12_emb64_bs256', type=str)
    ...
    model = SASRec_Rezero(model_para, device=args.device,).to(args.device)
            ...
            if numIters % args.eval_iter == 0:
                ...
                with torch.no_grad():
                    ...
                    if mrr > max_mrr:
                        ...
                        torch.save(model.state_dict(), args.save_dir + "/{}_{}_{}_{}.pkl".format(args.num_blocks, args.learning_rate, args.data_ratio, args.step))
                        early_stop = 0
                    else:
                        print("mrr_5:", mrr)
                        print("mrr_10:", mrr_10)
