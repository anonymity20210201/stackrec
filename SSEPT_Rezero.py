...
class TransformerLayer_Rezero(nn.Module):
    def __init__(self, hidden_size, num_heads, model_para, reader, layerid, layer_num, dropout_rate=0.5):
        ...
        self.SelfAttention = multihead_attention_rezero(model_para, reader, layerid, layer_num, hidden_size, num_units=self.hidden_size,
                                                 num_heads=self.num_heads, dropout_rate=self.dropout_rate,
                                                 causality=True, with_qk=False)
        self.ff = feedforward_rezero(model_para, reader, layerid, layer_num, num_units=[self.hidden_size, self.hidden_size], dropout_rate=self.dropout_rate)
...
class SSEPT_Rezero(nn.Module):
    def __init__(self, model_para, device='gpu'):
        ...
        rb = [TransformerLayer_Rezero(self.hidden_size, self.num_heads, self.model_para, self.reader, layerid, self.num_blocks, dropout_rate=self.dropout_rate) for layerid in range(self.num_blocks)]
