...
def nextitnet_residual_block_rezero(input_, dilation, layer_id, method,
    ...
    with tf.variable_scope(resblock_name, reuse=tf.AUTO_REUSE):
        ...
        if method == 'StackR':
            if layer_id >= layer_num / 2:
                ...
                variable_name = resblock_name + '/rez'
                new_variable_name = resblock_name + '/rez'
                initial_value = get_parameters(train, reader, variable_name, new_variable_name)
                rez = tf.get_variable('rez', [1],
    ...
    with tf.variable_scope(name):
        ...
        if method == 'StackR':
            if layer_id >= layer_num / 2:
                ...
                initial_value = get_parameters(trainable, reader, variable_name, new_variable_name)
                beta = tf.get_variable('beta', [int(shape[-1])],
                                    initializer=tf.constant_initializer(initial_value, verify_shape=True), trainable=trainable)
